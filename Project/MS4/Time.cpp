...
namespace sdds {
	...
	Time& Time::operator-=(const Time& D)
	{
		...
		if (negitive < 0) {
			this->m_min = (int)((trunc(this->m_min / 60) - trunc(D.m_min / 60)) + ceil(trunc(D.m_min / 60) / 24) * 24) * 60 + (this->m_min + D.m_min) % 60;
		}
	...
	Time Time::operator-(const Time& D) const
	{
		...
		int negitive = this->m_min - D.m_min;
		if (negitive < 0) {
			ret.m_min = (int)((trunc(this->m_min / 60) - trunc(D.m_min / 60)) + ceil(trunc(D.m_min / 60) / 24) * 24) * 60 + (this->m_min + D.m_min) % 60;
		}
